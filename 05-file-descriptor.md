...
また、[01-process](./01-process.md) では、プロセスが開いているファイルを「観察」しました
...
1. [ファイルディスクリプタとは何か](#ファイルディスクリプタとは何か)
2. [標準入出力と標準エラー出力](#標準入出力と標準エラー出力)
3. [カーネルの 3 層構造](#カーネルの-3-層構造)
4. [open() によるファイルオープン](#open-によるファイルオープン)
5. [read() と write()](#read-と-write)
6. [close() の重要性](#close-の重要性)
7. [lseek() による位置制御](#lseek-による位置制御)
8. [dup() と dup2() によるリダイレクト](#dup-と-dup2-によるリダイレクト)
9. [fork() と fd の継承](#fork-と-fd-の継承)
10. [用語集](#用語集)
11. [参考資料](#参考資料)
...
- <strong>パイプ</strong>（プロセス間でデータをやり取りする仕組み、[07-ipc](./07-ipc.md) で学習）
...
カーネルについては [01-process](./01-process.md) で説明しています
...
errno については、[01-process](./01-process.md) の「C 言語の読み方」で説明しています
...
size_t と ssize_t については、[01-process](./01-process.md) の「C 言語の読み方」で説明しています
...
execvp については [02-fork-exec](./02-fork-exec.md) で学んでいます
...
[02-fork-exec](./02-fork-exec.md) で学んだように、exec() 後も fd は維持されます

この性質のおかげで、シェルはリダイレクトを設定してから別のプログラムを実行できます

### スレッドとの違い（04-thread との関連）
...
次の [06-stdio](./06-stdio.md) では、標準入出力ライブラリを学びます

- printf() や scanf() は内部で何をしているか
- fopen() と open() の違い
- バッファリングの仕組み

また、fd の知識は [07-ipc](./07-ipc.md)（プロセス間通信）でも重要です
